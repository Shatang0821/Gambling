...
namespace FrameWork.Pool
{
    public class ObjectPool<T> where T : IPoolable, new()
    {
        ...
        ~ObjectPool()
        {
            ...
            if (RuntimeSize <= 0)
            {
                Debug.Log("ObjectPoolに問題が発生しました");
            }
        }
    ...
    public class UnityObjectPool
    {
        ...
        public void Clear()
        {
            foreach (var obj in _availableQueue)
            {
        ...
        {
            while (RuntimeSize > targetSize)
            {
                var obj = _availableQueue.Dequeue();
                GameObject.Destroy(obj);
            }

            // サイズが小さい場合、新しいオブジェクトを生成
            while (RuntimeSize < targetSize)
            {
                var obj = Copy(); // 新しいオブジェクトを作成
                _availableQueue.Enqueue(obj); // キューに追加
            }
        }
